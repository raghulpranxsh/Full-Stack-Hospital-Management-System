    ...
    if(date("Y-m-d",$appdate1)>=$cur_date){
      if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
        ...
        if(mysqli_num_rows($check_query)==0){
          ...
          if(!empty($pid) && !empty($fname) && !empty($lname) && !empty($gender) && !empty($email) && !empty($contact) && !empty($doctor) && !empty($docFees) && !empty($appdate) && !empty($apptime)) {
            
            // Use prepared statement to prevent SQL injection and handle quotes properly
            ...
            echo "<script>alert('Error: Missing required user information. Please log in again.');</script>";
          }
        } else {
